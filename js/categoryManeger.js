...
const selectedStoreNameForCategoriesEl = document.getElementById('selectedStoreNameForCategories');
...
export async function populateCategoryDropdown(storeId) {
    const categorySelect = document.getElementById('productCategory');
    if (!categorySelect) return;
    
    categorySelect.innerHTML = '<option value="">-- Carregando Categorias... --</option>';
    const categoriesRef = collection(db, 'mercerias', storeId, 'categorias');
...
categoryForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        populateCategoryDropdown(selectedStoreIdForCategories); // Atualiza o dropdown no form de produtos
    } catch (error) {
...
window.deleteCategory = async (categoryId, categoryName) => {
    // 1. Verificar se algum produto usa esta categoria
    ...
    if (productCount > 0) {
        alert(`Não é possível excluir a categoria "${categoryName}" pois ela está sendo usada por ${productCount} item(ns). Remova ou altere os itens antes de excluir a categoria.`);
        return;
    }

    // 2. Se nenhum item usa, prosseguir com a exclusão
    if (confirm(`Tem certeza que deseja excluir a categoria "${categoryName}"?`)) {
        try {
            const categoryDocRef = doc(db, 'mercerias', selectedStoreIdForCategories, 'categorias', categoryId);
            await deleteDoc(categoryDocRef);
            loadCategories(); // Recarrega a lista
            populateCategoryDropdown(selectedStoreIdForCategories); // Atualiza o dropdown
        } catch (error) {
...
// Função global para iniciar o gerenciamento de categorias
window.manageCategories = (storeId, storeName) => {
    selectedStoreIdForCategories = storeId;
    selectedStoreNameForCategoriesEl.textContent = `Gerenciando Categorias para: ${storeName}`;
